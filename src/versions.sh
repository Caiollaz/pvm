...
# Active version: $PVM_PHP_VERSION env > nearest .pvmrc > global file.
pvm_resolve_version() {
  if [ -n "${PVM_PHP_VERSION:-}" ]; then
    pvm_normalize_version "$PVM_PHP_VERSION"; return 0
  fi
  if pvm_find_rc_version; then return 0; fi
...
pvm_cmd_install() {
  [ $# -ge 1 ] || pvm_die "usage: pvm install <version>"
  pvm_require_docker
  local v; v="$(pvm_normalize_version "$1")"
  pvm_pull_version "$v"
  pvm_ok "PHP $v installed."
  if [ ! -f "$PVM_VERSION_FILE" ]; then
    pvm_set_global_version "$v"
    pvm_ok "Set global PHP version to $v."
  fi
}

pvm_cmd_uninstall() {
  [ $# -ge 1 ] || pvm_die "usage: pvm uninstall <version>"
  pvm_require_docker
  local v; v="$(pvm_normalize_version "$1")"
  pvm_image_present "$v" || pvm_die "PHP $v is not installed."
  docker image rm "$(pvm_image_for "$v")" >/dev/null \
    || pvm_die "Could not remove image for PHP $v."
  pvm_ok "PHP $v uninstalled."
}
...
pvm_cmd_list() {
  ...
  if [ -z "$versions" ]; then
    pvm_info "No PHP versions installed. Run: pvm install <version>"
    return 0
  fi
  local v
...
pvm_cmd_use() {
  local v="${1:-}"
  if [ -z "$v" ]; then
    v="$(pvm_find_rc_version)" \
      || pvm_die "No version given and no $PVM_RC_FILE found in this directory tree."
    pvm_info "Detected $PVM_RC_FILE → $v"
  fi
  v="$(pvm_normalize_version "$v")"
  pvm_image_present "$v" || pvm_die "PHP $v is not installed. Run: pvm install $v"
  pvm_set_global_version "$v"
  pvm_ok "Now using PHP $v."
}
...
pvm_cmd_current() {
  ...
  if v="$(pvm_resolve_version)"; then
    ...
    pvm_info "No PHP version selected. Run: pvm use <version>"
    return 1
  fi
}
